...
# Initialize client using new OpenAI SDK
...
class TokenPredictorApp:
    def __init__(self, root):
        ...
        # Prompt Entry
        prompt_frame = tk.Frame(frame_top, bd=1, relief="solid", bg="#f8f8ff")
        prompt_frame.pack(side=tk.LEFT, padx=(0, 20))
        ...
        tk.Label(prediction_frame, text="Next Token | Probability", bg="#e0ecff").pack(anchor="w", padx=10, pady=(5, 0))
        self.prediction_labels = []
        for _ in range(10):
            label = tk.Label(prediction_frame, text="", anchor="w", bg="#e0ecff", cursor="hand2")
        ...
        tk.Label(temp_frame, text="Model Temperature").pack(side=tk.LEFT, padx=(10, 5))
        self.temp_entry = tk.Entry(temp_frame, width=5, textvariable=self.temperature)
    ...
    def on_prompt_keyrelease(self, event):
        ...
        self.debounce_id = self.root.after(500, self.update_predictions)
